...
function WithdrawModal({ sdkInstance, onWithdrawComplete }) {
  ...
  const withdraw = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const recipientRef = (e.currentTarget.elements.namedItem("recipient") as HTMLInputElement);
  ...
  return (
    <div className="flex-column">
      {
        withdrawStage == "success" ? <h3>Withdraw successful!</h3> :
        withdrawStage == "failure" ? <h3>Withdraw failed - try again!</h3> :
        ...
        <form onSubmit={withdraw}>
          ...
          <input required name="recipient" placeholder="Ethereum address" value={recipient} onChange={e => setRecipient(e.target.value)} />
